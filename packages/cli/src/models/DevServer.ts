...
export class DevServer {
  ...
  public async start() {
    ...
    const application = ApplicationMetadata.set(
      await _import<Function>(bluishConfiguration.application && path.join(input, bluishConfiguration.application))
    )
    ...
    const watchers = {
      ...
      functions: new ChokidarWatcher(bluishConfiguration.functions, {
        cwd: input
      })
    }

    watchers.functions.on('change', () => this.clearInputModules())
    ...
    const fork = new AzureFuncSpawn('start', {
      cwd: output,
      flags: { port },
      stdio: true
    })
